		...
		try:
			...
			if beatmapInfo is not None and beatmapInfo != False and s.passed:
				...
				if newScoreboard.personalBestRank == 1 and s.completed == 3 and restricted == False:
					annmsg = "[{}/?u={} {}] achieved rank #1 on [https://osu.ppy.sh/b/{} {}] ({}) {}pp".format(
						url,
						userID,
					...
					if type == "regular":
						...
						embed.set_desc("Achieved a Score on mode **{}**, {} +{} on regular!".format(
						gameModes.getGamemodeFull(s.gameMode),
						beatmapInfo.songName.encode().decode("ASCII", "ignore"),
						ScoreMods
						))
						...
						embed.add_field(name='Actual rank: {}'.format(
						rankInfo["currentRank"]
						),value='[Download Link](http://storage.hatsunai.me/d/{})'.format(
						beatmapInfo.beatmapSetID
						))
						embed.set_image('https://assets.ppy.sh/beatmaps/{}/covers/cover.jpg'.format(
						beatmapInfo.beatmapSetID
